  ...
  presets: [
    {
      id: "movie4k",
      icon: "🎬",
      values: { fileSize: 15, fileSizeUnit: "GB", speed: 100, speedUnit: "mbps" },
    },
    {
      id: "backup100gb",
      icon: "💾",
      values: { fileSize: 100, fileSizeUnit: "GB", speed: 50, speedUnit: "mbps" },
    },
    {
      id: "gameDownload",
      icon: "🎮",
      values: { fileSize: 80, fileSizeUnit: "GB", speed: 300, speedUnit: "mbps" },
    },
    {
      id: "cloudBackup1tb",
      icon: "☁️",
      values: { fileSize: 1, fileSizeUnit: "TB", speed: 20, speedUnit: "mbps" },
    },
  ],
  ...
  t: {
    en: {
      ...
      inputs: {
        ...
        fileSizeUnit: {
          label: "Size Unit",
          helpText: "Unit for the file size",
          options: {
            "KB": "KB (kilobytes)",
            "MB": "MB (megabytes)",
            "GB": "GB (gigabytes)",
        ...
        speedUnit: {
          label: "Speed Unit",
          helpText: "Unit for the connection speed",
          options: {
      ...
      formats: {
        summary: "Transferring {fileSize} {fileSizeUnit} at {speed} {speedUnit} takes approximately {transferTime}.",
      },
...
export function calculateTransferTime(data: {
  ...
  const fileSizeUnit = values.fileSizeUnit as string;
  const speed = values.speed as number | null;
  const speedUnit = values.speedUnit as string;

  if (fileSize === null || speed === null || fileSize <= 0 || speed <= 0) {
    return { values: {}, formatted: {}, summary: "", isValid: false };
  }

  const fileSizeBytes = fileSize * (SIZE_TO_BYTES[fileSizeUnit] || 1e6);
  ...
  const summary = (f.summary || "Transferring {fileSize} {fileSizeUnit} at {speed} {speedUnit} takes approximately {transferTime}.")
    .replace("{fileSize}", fileSize.toString())
    .replace("{fileSizeUnit}", fileSizeUnit)
