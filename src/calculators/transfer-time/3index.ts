...
// ─────────────────────────────────────────────────────────────────────────────
// Transfer Time Calculator — V4 with Overhead, Interface Select & Comparison
// ─────────────────────────────────────────────────────────────────────────────
...
export const transferTimeConfig: CalculatorConfigV4 = {
  ...
  presets: [
    ...
    },
    {
      id: "osUpdate",
      icon: "💻",
      values: { fileSize: 5, speed: 50, overheadPercent: 10, connectionType: "custom" },
    },
    {
      id: "cloudBackup",
      icon: "☁️",
  ...
  t: {
    en: {
      ...
      formats: {
        summary: "Transferring {fileSize} at {speed} ({overhead}% overhead) takes approximately {transferTime}.",
      },
      ...
      infoCards: {
        metrics: {
          ...
          items: [
            "ISPs advertise speed in megabits (Mbps), not megabytes (MB/s)",
            "1 byte = 8 bits — so 100 Mbps = 12.5 MB/s maximum",
            "Protocol overhead (TCP/IP, TLS) typically reduces speed by 5–15%",
            "Upload speeds are usually 5–10× slower than download speeds",
          ],
        },
        details: {
          title: "Quick Reference",
          items: [
            "100 Mbps connection ≈ 11 MB/s real-world throughput",
            "Wi-Fi actual speed is typically 30–70% of rated speed",
            "USB 3.0 (5 Gbps) real-world: ~400 MB/s with SSD, ~100 MB/s with HDD",
            "NVMe SSD can read at 3,500 MB/s — faster than most network connections",
          ],
        },
        tips: {
...
// ─────────────────────────────────────────────────────────────────────────────
// Conversion maps — IDs match the unit registry (UPPERCASE as in field-presets)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Formatting helpers — fmtNum for fixed units, auto* for auto-scale
// ─────────────────────────────────────────────────────────────────────────────
...
function smartBits(bits: number): string {
  if (bits >= 1e12) return `${fmtNum(bits / 1e12)} Tbps`;
  if (bits >= 1e9) return `${fmtNum(bits / 1e9)} Gbps`;
  if (bits >= 1e6) return `${fmtNum(bits / 1e6)} Mbps`;
  if (bits >= 1e3) return `${fmtNum(bits / 1e3)} Kbps`;
  return `${fmtNum(bits)} bps`;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// CALCULATE — reads units from fieldUnits (unitType dropdowns)
// ─────────────────────────────────────────────────────────────────────────────
...
export function calculateTransferTime(data: {
  ...
  // Read units from unitType dropdowns (IDs are uppercase: GB, MB, TB, Mbps, etc.)
  ...
  // Derived speeds
  const speedMbps = rawBps / 1e6;
  const speedMBps = rawBps / 8e6;
  ...
  // Summary
  const summary = (f.summary || "Transferring {fileSize} at {speed} ({overhead}% overhead) takes approximately {transferTime}.")
  ...
  const tableData = comparisonInterfaces.map((key) => {
    ...
    return {
      ...
      ratedSpeed: smartBits(ifaceBps),
      effectiveSpeed: `${fmtNum(ifaceEffective / 8e6)} MB/s`,
  ...
  return {
    values: {
      ...
      speedMbps,
      speedMBps,
      effectiveSpeed: effectiveMBps,
      fileSizeBytes,
    },
    formatted: {
      transferTime,
      totalSeconds: fmtNum(Math.round(totalSeconds * 100) / 100),
      speedMbps: `${fmtNum(speedMbps)} Mbps`,
      speedMBps: `${fmtNum(speedMBps)} MB/s`,
      effectiveSpeed: `${fmtNum(effectiveMBps)} MB/s`,
