      ...
      {!fullscreen && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div><h1 className="text-lg sm:text-lg sm:text-xl sm:text-2xl font-bold text-slate-900">{isEditing ? "Edit Post" : "New Post"}</h1><p className="text-slate-500 text-sm">Markdown + HTML · 5 languages</p></div>
          </div>
        ...
        <div className={fullscreen ? "flex-1 flex flex-col overflow-hidden" : "xl:col-span-2 space-y-5"}>
          ...
          {!fullscreen && (<>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">{LANGS.map((l) => { const st = ls(l.code); const c = form[lf("content", l.code)] as string; const cw = c.trim().split(/\s+/).filter(Boolean).length; return (<button key={l.code} onClick={() => setTab(l.code)} className={`p-3 rounded-xl border text-center transition-all ${tab === l.code ? "border-blue-300 bg-blue-50 ring-1 ring-blue-200" : st === "complete" ? "border-emerald-200 bg-emerald-50 hover:bg-emerald-100" : st === "partial" ? "border-amber-200 bg-amber-50 hover:bg-amber-100" : "border-slate-200 bg-slate-50 hover:bg-slate-100"}`}><span className="text-2xl block mb-1">{l.flag}</span><p className="text-xs font-bold text-slate-700">{l.code.toUpperCase()}</p><p className={`text-xs mt-1 ${st === "complete" ? "text-emerald-600" : st === "partial" ? "text-amber-600" : "text-slate-400"}`}>{st === "complete" ? `✓ ${cw}w` : st === "partial" ? "Partial" : "Empty"}</p></button>); })}</div>
            </div>
          </>)}
        </div>
