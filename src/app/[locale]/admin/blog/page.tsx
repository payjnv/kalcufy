...
function getTranslationStatus(post: Post) {
  return LANGS.map((lang) => {
    if (lang.code === "en") return { ...lang, done: true }; // EN always required
    const titleKey = `title${lang.code.charAt(0).toUpperCase() + lang.code.slice(1)}` as keyof Post;
    const contentKey = `content${lang.code.charAt(0).toUpperCase() + lang.code.slice(1)}` as keyof Post;
    const hasTitle = !!post[titleKey];
    const hasContent = !!post[contentKey];
    return { ...lang, done: hasTitle && hasContent };
  });
}

function getTranslationCount(post: Post): number {
  return getTranslationStatus(post).filter((l) => l.done).length;
}

// ── SEO score helper ──
function getSeoScore(post: Post): { score: number; issues: string[] } {
  const issues: string[] = [];
  let score = 0;
  ...
  if (post.excerptEn && post.excerptEn.length >= 50) score += 25;
  else if (post.excerptEn) { score += 10; issues.push("Excerpt too short"); }
  else issues.push("No excerpt");

  if (post.metaTitleEn && post.metaTitleEn.length >= 30) score += 25;
  else if (post.metaTitleEn) { score += 10; issues.push("Meta title too short"); }
  else issues.push("No meta title");

  if (post.metaDescriptionEn && post.metaDescriptionEn.length >= 80) score += 25;
  else if (post.metaDescriptionEn) { score += 10; issues.push("Meta description too short"); }
  else issues.push("No meta description");

  return { score, issues };
}

function SeoIndicator({ post }: { post: Post }) {
  const { score, issues } = getSeoScore(post);
    ...
    <div className="relative group">
      <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${color}`}>
        <svg className="w-3.5 h-3.5" viewBox="0 0 20 20">
          ...
          <circle
            cx="10" cy="10" r="8" fill="none" className={ring} strokeWidth="2"
            strokeDasharray={`${(score / 100) * 50.26} 50.26`}
            strokeLinecap="round"
      ...
      {issues.length > 0 && (
        <div className="absolute z-50 bottom-full left-0 mb-1 hidden group-hover:block w-48 bg-slate-900 text-white text-xs rounded-lg p-2 shadow-lg">
          <p className="font-medium mb-1">SEO Issues:</p>
          {issues.map((issue, i) => (
...
// ── Translation Badges Component ──
function TranslationBadges({ post }: { post: Post }) {
  const statuses = getTranslationStatus(post);
  return (
      ...
      {statuses.map((lang) => (
        <span
          ...
          className={`text-xs px-1 py-0.5 rounded ${
            lang.done ? "opacity-100" : "opacity-30 grayscale"
          }`}
          title={`${lang.code.toUpperCase()}: ${lang.done ? "Translated" : "Missing"}`}
        >
          {lang.flag}
        </span>
      ))}
...
export default function AdminBlogPage() {
  ...
  const [stats, setStats] = useState<Stats>({
    ...
    totalViews: 0, viewsThisMonth: 0, viewsLastMonth: 0,
    avgReadingTime: 0, topPost: null, viewsByDay: [],
  });
  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      ...
      const postsRes = await fetch(`/api/admin/blog?${params}`, { credentials: "include" });
      const postsData = await postsRes.json();
      setPosts(postsData.posts || []);
      setPagination(postsData.pagination || { page: 1, limit: 20, total: 0, totalPages: 0 });

      const allPosts = postsData.posts || [];
      const avgReadingTime = allPosts.length > 0
        ? Math.round(allPosts.reduce((sum: number, p: Post) => sum + (p.readingTime || 5), 0) / allPosts.length)
        : 0;
      const sortedByViews = [...allPosts].sort((a: Post, b: Post) => b.views - a.views);

      setStats({
        total: postsData.stats?.total || allPosts.length,
        ...
        viewsThisMonth: postsData.stats?.viewsThisMonth || 0,
        viewsLastMonth: postsData.stats?.viewsLastMonth || 0,
        avgReadingTime,
        topPost: sortedByViews[0] || null,
        viewsByDay: postsData.stats?.viewsByDay || [],
      });
    ...
    try {
      ...
      if (res.ok) {
        setPosts((prev) =>
          prev.map((p) =>
            p.id === postId
              ? { ...p, status: newStatus as Post["status"], publishedAt: newStatus === "PUBLISHED" ? new Date().toISOString() : p.publishedAt }
              : p
          )
        );
        // Update stats counts
        setStats((prev) => {
          ...
          if (!old) return prev;
          const updated = { ...prev };
          if (old.status === "PUBLISHED") updated.published--;
          if (old.status === "DRAFT") updated.drafts--;
          if (old.status === "SCHEDULED") updated.scheduled--;
          if (newStatus === "PUBLISHED") updated.published++;
          if (newStatus === "DRAFT") updated.drafts++;
  ...
  const Sparkline = ({ data }: { data: { views: number }[] }) => {
    if (!data || data.length === 0) return <div className="h-8 flex items-center text-xs text-slate-400">No data</div>;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl p-6 text-white">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-center gap-4 text-sm text-slate-400">
                ...
                <TranslationBadges post={stats.topPost} />
              </div>
              <Link
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <div className="space-y-2">
            <Link href="/en/admin/blog/new" className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors group">
            ...
            <Link href="/en/admin/blog/categories" className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors group">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                categoryFilter === cat.slug ? "ring-2 ring-offset-2 ring-blue-500" : ""
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
          <div className="flex flex-col sm:flex-row gap-3 flex-1 w-full lg:w-auto">
            ...
            <select
              ...
              className="px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50"
            >
            ...
            >
              ...
              {categories.map((cat) => (
                <option key={cat.id} value={cat.slug}>{cat.icon} {cat.nameEn}</option>
              ))}
            </select>
          </div>
          <div className="flex items-center gap-2">
            {selectedPosts.length > 0 && (
              <div className="flex items-center gap-2 mr-2 pr-2 border-r border-slate-200">
                ...
                <button onClick={() => handleBulkAction("published")} className="px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-lg text-sm font-medium hover:bg-emerald-200">
                  Publish
                </button>
                <button onClick={() => handleBulkAction("delete")} className="px-3 py-1.5 bg-red-100 text-red-700 rounded-lg text-sm font-medium hover:bg-red-200">
                  Delete
                </button>
              </div>
            )}
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600">Languages</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600">Category</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600">Status</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600">SEO</th>
                  ...
                  <tr key={post.id} className={`hover:bg-slate-50 transition-colors ${selectedPosts.includes(post.id) ? "bg-blue-50" : ""}`}>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-sm text-slate-500 truncate max-w-xs">/{post.slugEn}</p>
                        </div>
                      </div>
                    </td>
                    {/* Translation Badges */}
                    <td className="py-3 px-4">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-xs text-slate-400">{getTranslationCount(post)}/5</span>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      {post.category ? (
                    ...
                    <td className="py-3 px-4">
                      <select
                        value={post.status}
                        onChange={(e) => handleQuickStatusChange(post.id, e.target.value)}
                        disabled={changingStatus === post.id}
                        className={`text-sm font-medium rounded-lg px-2.5 py-1 border-0 cursor-pointer focus:ring-2 focus:ring-blue-500 ${statusConfig[post.status].color} ${changingStatus === post.id ? "opacity-50" : ""}`}
                      >
                        <option value="DRAFT">✏️ Draft</option>
                        <option value="PUBLISHED">✅ Published</option>
                        <option value="SCHEDULED">📅 Scheduled</option>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center justify-end gap-1">
                        <Link href={`/en/blog/${post.slugEn}`} target="_blank" className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors" title="View">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                          </svg>
                        </Link>
                        <button onClick={() => handleDuplicate(post)} className="p-2 text-slate-400 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-colors" title="Duplicate">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                          </svg>
                        </button>
                        <Link href={`/en/admin/blog/${post.id}`} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                          </svg>
                        </Link>
                        <button onClick={() => handleDelete(post.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-30 transition-colors"
            >
              Previous
            </button>
              ...
              return (
                <button
                  ...
                  className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${
                    p === pagination.page ? "bg-blue-600 text-white" : "hover:bg-slate-100 text-slate-600"
                  }`}
                >
                  {p}
                </button>
