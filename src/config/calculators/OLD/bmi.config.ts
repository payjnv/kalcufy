...
export function calculateBMI(data: {
  ...
  if (unitSystem === "metric") {
    ...
    if (bmi < healthyBmiMin) {
      ...
      weightToHealthy = `{{t:results.gainToHealthy|Gain}} ${toGain.toFixed(1)} kg {{t:results.toReachHealthy|to reach healthy range}}`;
    } else if (bmi > healthyBmiMax) {
      const toLose = weightKg - healthyWeightMaxKg;
      weightToHealthy = `{{t:results.loseToHealthy|Lose}} ${toLose.toFixed(1)} kg {{t:results.toReachHealthy|to reach healthy range}}`;
    } else {
      weightToHealthy = "{{t:results.inHealthyRange|You're in the healthy range! 🎉}}";
    }
  } else {
    ...
    if (bmi < healthyBmiMin) {
      ...
      weightToHealthy = `{{t:results.gainToHealthy|Gain}} ${Math.round(toGain)} lbs {{t:results.toReachHealthy|to reach healthy range}}`;
    } else if (bmi > healthyBmiMax) {
      const toLose = weightLbs - healthyWeightMaxLbs;
      weightToHealthy = `{{t:results.loseToHealthy|Lose}} ${Math.round(toLose)} lbs {{t:results.toReachHealthy|to reach healthy range}}`;
    } else {
      weightToHealthy = "{{t:results.inHealthyRange|You're in the healthy range! 🎉}}";
    }
  }
  ...
  return {
    ...
    summary: `{{t:results.summaryStart|Your BMI is}} ${bmi.toFixed(1)}, {{t:results.summaryClassified|which is classified as}} ${category.label}. ${weightToHealthy}`,
    isValid: true,
    metadata: {
      bmiScale,
